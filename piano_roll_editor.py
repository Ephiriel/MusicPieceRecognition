...
class PianoRoll(QtWidgets.QGraphicsScene):
    ...
    def drawHeader(self):
        self.header = QtWidgets.QGraphicsRectItem(0, 0, self.grid_width, self.header_height)
        ...
        for i in range(self.end_octave - self.start_octave, self.start_octave - self.start_octave, -1):
            for j in range(self.notes_in_octave, 0, -1):
                if j in black_notes:
                    ...
                    key.setPos(0, self.note_height * j + self.octave_height * (i - 1))
                if j == 12:
                    label = QtWidgets.QGraphicsSimpleTextItem('{}{}'.format(labels[j - 1], self.end_octave - i), key)
                    label.setPos(18, 6)
                    label.setFont(piano_label)
                self.piano_keys.append(key)

    def drawGrid(self):
        scale_bar = QtWidgets.QGraphicsRectItem(0, 0, self.grid_width, self.note_height, self.piano)
    ...
    def drawMidiFile(self, mf: MidiFile = None):
        ...
        num, denom = self.midifile.get_time_signature()
        resolution = self.midifile.get_resolution()

        self.setMeasures(1)
        self.setTimeSig(str(num) + "/" + str(denom))
        self.refreshScene()

        for idx in reversed(range(len(notes))):
            self.drawNote(notes[idx], positions[idx], durations[idx], note_velocity=20, pos_scale=1 / resolution,
                                length_scale=1 / (4 * resolution))
